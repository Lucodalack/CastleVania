...
void Bone::Update(float DeltaTime)
{
	...
	if (left)
	{
		if (_x>_mid)
		{
			_vx = -0.2f;
			//vx cho cả 2 bên
			// mid tọa độ theo trục x (là điểm chính giữa con simon và con con skeleton)
			// top là tọa độ theo trục y của con simon
			//
			if (_y < _top - 20)
			{
				_vy = -0.4f;
	...
	//this->_x += (this->_vx * DeltaTime*cos(3.14/3));
	//this->_y += (_vx * DeltaTime*sin(3.14 / 3) - 0.5*this->_vy * DeltaTime*DeltaTime);
