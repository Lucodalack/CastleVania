...
#pragma region __XU_LY_CHUYEN_DONG__
	...
	if (_going) //neu dang di quay lai
	{
		...
		/*if ((this->_x >= Simon::getCurrentSimon()->_x  && !_turnlef)
			|| (this->_x < Simon::getCurrentSimon()->_x  && _turnlef)
			|| d >= GCamera::getCurrentCamera()->width 
			|| -d >= GCamera::getCurrentCamera()->width){
			_flying = false;
			this->_sprite->Done = -1;
		}*/
			...
			if (
				   (_turnlef && ((this->_x < Simon::getCurrentSimon()->_x && this->_y >= simony && this->_y <= simony+simonh)
					|| this->_x < GCamera::getCurrentCamera()->x )) // dang bay qa trai, dung con simon hoac bay ra khoi camera
				|| (!_turnlef && ((this->_x >= Simon::getCurrentSimon()->_x && this->_y >= simony && this->_y <= simony + simonh)
